...
def freq2ne(freq,Te=1000.0,f0=450.0e6,B=50000e-9,alph=0.0):
    ...
    ne = 4.0 * math.pi**2 * v_electronmass * v_epsilon0 / v_elemcharge**2 \
            * (np.power(freq,2.0)-3 * k**2 / 4.0 / math.pi**2 * \
            v_Boltzmann * Te / v_electronmass - fc**2 * np.sin(alph)**2)

    return ne

def chapman_func(z,Hch,nmax,zmax):

    ne=nmax*np.exp(0.5*(1.0-(z-zmax)/Hch-np.exp((zmax-z)/Hch)))
...
def chap_fitfun(parameter,altitude,data,scaler,PW,mode=0):
    ...
    althr = np.arange(altitude[0]-PW,altitude[-1]+PW,100)

    # model
    m = chapman_func(althr,thch,tnmax,tzmax)

    # smear model
    m2 = np.zeros(data.shape,dtype=data.dtype)
    for rr in range(altitude.shape[0]):
        I = np.where((althr >= (altitude[rr] - PW/2.0)) & (althr <=
                                            (altitude[rr]+PW/2.0)))[0]
        m2[rr] = np.mean(m[I] * (altitude[rr]**2.0) / np.power(althr[I],2.0))
...
def canny(image, high_threshold, low_threshold):
    ...
    _N  = np.array([[0, 1, 0],[0, 0, 0],[0, 1, 0]], dtype=bool)
    _NE = np.array([[0, 0, 1],[0, 0, 0],[1, 0, 0]], dtype=bool)
    _W  = np.array([[0, 0, 0],[1, 0, 1],[0, 0, 0]], dtype=bool)
    _NW = np.array([[1, 0, 0],[0, 0, 0],[0, 0, 1]], dtype=bool)
...
#    canny_edges = ndimage.binary_dilation(high, structure=np.ones((3,3)),
...
def read_pl_file(fname):
    ...
    for icnt,line in enumerate(lines):
        ...
        if icnt==0 and (len(line)==4 or len(line)==5):
            bmcode=np.array(line).astype('float64')
...
#    INT='_ac_5min';     LONGPULSE=0; doChap=0; usePower=0; Ilen=3; useAltPl=0
    ...
    pl_type = {'LP':{'INT': '_lp_5min', 'LONGPULSE': 1, 'doChap': 0,
                     'usePower': 0, 'Ilen': 2, 'useAltPl': 1,
                     'filter':'*_lp_5min_.txt'},
               'AC':{'INT': '_ac_5min', 'LONGPULSE': 0, 'doChap': 0,
                     'usePower': 0, 'Ilen': 2, 'useAltPl': 1,
                     'filter':'*_ac_5min_.txt'}
              }

    # pl_type = {'LP':{'INT': '_lp_30sec', 'LONGPULSE': 1, 'doChap': 0,
    #                  'usePower': 0, 'Ilen': 2, 'useAltPl': 1,
    #                  'filter':'*_lp_30sec_.txt'},
    #            'AC':{'INT': '_ac_30sec', 'LONGPULSE': 0, 'doChap': 0,
    #                  'usePower': 0, 'Ilen': 2, 'useAltPl': 1,
    #                  'filter':'*_ac_30sec_.txt'}
    #           }

    # pl_type = {'LP':{'INT': '_lp_1min', 'LONGPULSE': 1, 'doChap': 0,
    #                  'usePower': 0, 'Ilen': 1, 'useAltPl': 0,
    #                  'filter':'*_lp_1min_.txt'},
    #            'AC':{'INT': '_ac_3min', 'LONGPULSE': 0, 'doChap': 0,
    #                  'usePower': 0, 'Ilen': 3, 'useAltPl': 0,
    #                  'filter':'*_ac_3min_.txt'}
    #             }

    ST='';
    caldir='cal-0/'
    start_dir = '/' #/home/ashton/heaviside'
    top_cal_dir = os.path.join(start_dir,
            'Volumes/AMISR_PROCESSED/calibration/AMISR/calibration_PFISR')
    print(top_cal_dir)
    print("\n\n\n\n\n")
...
def cal_pl(EXP,INT,LONGPULSE,PLfile,ILfname,ILdir,doChap=-1,usePower=-1,ST='',
           Ilen=10,useAltPl=0):
...
#    PLfile='cal-0/20071011.011-65162-plline-000.txt'
    ...
    IlTime = np.mean(ILdata['/Time']['UnixTime'],axis=1)
    ...
    kperp = np.sqrt(np.power(ILdata['/Geomag']['kpe'],2.0)
            + np.power(ILdata['/Geomag']['kpn'],2.0))
    alph = math.pi/2 - np.arcsin(kpar/np.sqrt(np.power(kpar,2.0)
           + np.power(kperp,2.0))).real
    ...
    timepl = np.array(timepl)
    pl = np.array(pl)
    PW = PW * np.sin(bmcode[2] * np.pi/180.0)

    Ibeam=np.where(BMCODES[:,0] == bmcode[0])[0]
    if len(Ibeam) == 0:
        print('cant find beam!')
        sys.exit(-1)

    # get nemax from IL
    Itime = np.where((IlTime>=(timepl.min()-1000))
                    & (IlTime<=(timepl.max()+1000)))[0]
    timeil = IlTime[Itime]
    NeMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    dNeMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    htMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    TeMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    TiMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    TrMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    alphMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    BabsMax = np.zeros(timeil.shape,dtype='float64') * np.nan
    TxPow = np.zeros(timeil.shape,dtype='float64') * np.nan
    for aa in range(len(Itime)):
        ra = Itime[aa]

        TxPow[aa] = TxPower[ra]

        taltpower = np.squeeze(AltPower[Ibeam,:])
        Ialtpower = np.where((taltpower >= ALTMIN) & (taltpower <= ALTMAX))[0]
        taltpower = taltpower[Ialtpower]
        tprofpower = np.squeeze(NePower[ra,Ibeam,:])[Ialtpower]

        talt = np.squeeze(Alt[Ibeam,:])
        Ialt = np.where((talt>=ALTMIN) & (talt<=ALTMAX))[0]
        talt = talt[Ialt]

        tprof = np.squeeze(Ne[ra,Ibeam,:])[Ialt]
        dtprof = np.squeeze(dNe[ra,Ibeam,:])[Ialt]

        tte = np.squeeze(Te[ra,Ibeam,:])[Ialt]
        tti = np.squeeze(Ti[ra,Ibeam,:])[Ialt]
        talph = np.squeeze(alph[Ibeam,:])[Ialt]
        tBabs = np.squeeze(Babs[Ibeam,:])[Ialt]
        ttr = tte/tti
...
#        try:
        if 1==1:
            ...
            if useAltPl:
                ...
                NeMax[aa] = scipy.interpolate.interp1d(
                        ttalt,ttprof,bounds_error=0)(htMax[aa])
                dNeMax[aa] = scipy.interpolate.interp1d(
                        talt,dtprof,bounds_error=0)(htMax[aa])
                TeMax[aa] = scipy.interpolate.interp1d(
                        talt,tte,bounds_error=0)(htMax[aa])
                TrMax[aa] = scipy.interpolate.interp1d(
                        talt,ttr,bounds_error=0)(htMax[aa])

            else:

                # get maximum Ne and altitude of F peak.
                I = np.where((ttprof==ttprof.max()))[0]
    #            pylab.plot(ttprof,ttalt)
                ...
                p = np.polyfit(ttalt[I - Ilen:I + Ilen + 1],
                       ttprof[I - Ilen:I + Ilen + 1], 2, rcond=None, full=False)
                htMax[aa] = -p[1]/2.0/p[0]
                NeMax[aa] = p[0] * htMax[aa]**2 + p[1] * htMax[aa]+p[2]
                if NeMax[aa] > 1.0e13:
                    NeMax[aa] = 1.0e11
                dNeMax[aa] = np.std(ttprof[I - Ilen:I + Ilen + 1]
                        - np.polyval(p,ttalt[I - Ilen:I + Ilen + 1]))
    #            pylab.plot(np.polyval(p,ttalt[I-Ilen:I+Ilen+1]),
    #                  ttalt[I-Ilen:I+Ilen+1],'.k-')
    #            pylab.plot(ttprof[I-Ilen:I+Ilen+1]-np.polyval(
    #                  p,ttalt[I-Ilen:I+Ilen+1]),ttalt[I-Ilen:I+Ilen+1],'.b-')

                if doChap:
                    scaler=np.array([1.0e11,1.0e5,1.0e4])
                    params0=np.array([NeMax[aa],htMax[aa],thch])
                    (x,cov_x,infodict,mesg,ier) = scipy.optimize.leastsq(
                            chap_fitfun, params0/scaler,
                            (ttalt[I-Ilen:I+Ilen+1],
                            ttprof[I-Ilen:I+Ilen+1],scaler,PW),
                            full_output=1, epsfcn=1.0e-5, ftol=1.0e-5,
                            xtol=1.0e-5, gtol=0.0, maxfev=15*params0.shape[0],
                            factor=0.5,diag=None)
                    tz = np.arange(ttalt[I-Ilen],ttalt[I+Ilen+1],100)
...
#            xxx
    ...
    # get sys constant
    pl_ne=np.zeros(timepl.shape,dtype='float64')
    il_ne=np.zeros(timepl.shape,dtype='float64')
    il_dne=np.zeros(timepl.shape,dtype='float64')
    il_tr=np.zeros(timepl.shape,dtype='float64')
    for isamp in range(timepl.shape[0]):

        # find the closest time
        tmp=np.absolute(timeil-timepl[isamp])
        I=np.where(tmp==tmp.min())[0]
        I=I[0]
    ...
    Ksys_med=np.nanmedian(KsysCor)
    #    Ksys_std=np.nanmedian(KsysErr)
    Ksys_std=np.nanstd(KsysCor)
...
#   pylab.plot(timeil,TrMax)
    ...
    fH.write('%f %f\n' % (timepl[0],timepl[-1]))
    fH.write('%f %f\n' % (timeil[0],timeil[-1]))
    fH.write('%f %f\n' % (np.nanmedian(il_ne),np.nanmedian(il_tr)))
    fH.write('%f\n' % (np.nanmean(TxPow)))
    fH.write('\n')
...
def plot_pl():
    ...
    for Ifile in range(NFILES):
        ...
        if Ifile==0:
            rng=np.squeeze(output1['/PLFFTS/Data/Spectra']['Range'])
            fr_off=np.absolute(output1['/Tx']['Frequency'][0,0]-output1['/Rx']['Frequency'][0,0])
            if fr_off==output1['/Tx']['Frequency'][0,0]:
                fr_off=6.5e6
            freq_up=np.squeeze(output1['/PLFFTS/Data/Spectra']['Frequency'])+fr_off

            pl_spect_up=output1['/PLFFTS/Data/Spectra']['Data']
            bmcodes_up=output1['/PLFFTS/Data']['Beamcodes']
            time_up=np.mean(output1['/Time']['UnixTime'],axis=1)

            if dualpl:
                freq_dn=np.squeeze(output2['/PLFFTS/Data/Spectra']['Frequency'])+fr_off
                pl_spect_dn=output2['/PLFFTS/Data/Spectra']['Data']
                bmcodes_dn=output2['/PLFFTS/Data']['Beamcodes']
                time_dn=np.mean(output2['/Time']['UnixTime'],axis=1)
        else:
            pl_spect_up=np.concatenate((pl_spect_up,output1['/PLFFTS/Data/Spectra']['Data']),axis=0)
            bmcodes_up=np.concatenate((bmcodes_up,output1['/PLFFTS/Data']['Beamcodes']),axis=0)
            time_up=np.concatenate((time_up,np.mean(output1['/Time']['UnixTime'],axis=1)),axis=0)
            if dualpl:
                pl_spect_dn=np.concatenate((pl_spect_dn,output2['/PLFFTS/Data/Spectra']['Data']),axis=0)
                bmcodes_dn=np.concatenate((bmcodes_dn,output2['/PLFFTS/Data']['Beamcodes']),axis=0)
                time_dn=np.concatenate((time_dn,np.mean(output2['/Time']['UnixTime'],axis=1)),axis=0)
    # deal the data
    ...
    for jjj in range(Nbeams):
        ...
        tpl_spect_up=np.squeeze(pl_spect_up[:,jjj,:,:])
        tpl_spect_up=tpl_spect_up-np.repeat(tpl_spect_up[:,:,-1][:,:,np.newaxis],tpl_spect_up.shape[2],axis=2) 
        tpl_spect_up[np.where(tpl_spect_up<0.0)]=0.0
        if dualpl:
            tpl_spect_dn=np.squeeze(pl_spect_dn[:,jjj,:,:])
            tpl_spect_dn=tpl_spect_dn-np.repeat(tpl_spect_dn[:,:,-1][:,:,np.newaxis],tpl_spect_dn.shape[2],axis=2)
            tpl_spect_dn[np.where(tpl_spect_dn<0.0)]=0.0
        #
        alt_pl=rng*np.sin(bmcode[2]*math.pi/180)
    ...
    while not done:
        ...
        for Ifile in range(len(tfiles_up)):
            ...
            if Ifile==0:
                ...
                if dualpl:
                    fr_off=np.absolute(output2['/Tx']['Frequency'][0,0]-output2['/Rx']['Frequency'][0,0])
                    if fr_off==output1['/Tx']['Frequency'][0,0]:
                        fr_off=FR_OFF[1]
                    freq_dn=np.squeeze(output2['/PLFFTS/Data/Spectra']['Frequency'])+fr_off
                    pl_spect_dn=output2['/PLFFTS/Data/Spectra']['Data']
        ...
        beamcodes=np.sort(bmcodes_up[0,:])
        Nbeams=beamcodes.shape[0]
        BMCODES=np.zeros((Nbeams,4),dtype='Float64') # beamcode table (beamcode,az,el,ksys)
        for i in range(Nbeams):
            I=np.where(BeamcodeMap[:,0]==beamcodes[i])[0]
        ...
        for jjj in range(Nbeams):
            ...
            # get beamcode
            bmcode=BMCODES[jjj,:]
            rrI=np.where(bmcode[0]==beams2do)[0]
            ...
            if len(rrI)>=1:
                ...
                tmp=np.absolute(alt_pl-htstart)
                Ialt=np.where(tmp==tmp.min())[0]
                pl_up=np.squeeze(tpl_spect_up[:,:,Ialt])
                if dualpl:
                    pl_dn=np.squeeze(tpl_spect_dn[:,:,Ialt])
                if dualpl:
                    if flip==0:
                        pl_dn=np.fliplr(pl_dn)
                if flip==1:
                        pl_up=np.fliplr(pl_up) # down is really up!!
...
if __name__ == '__main__':
    ...
    if filterBySza:
        ...
        try:
            files_up=np.loadtxt(os.path.join(ODIR,'SZAlist_up.txt'),dtype='string',delimiter='\n').tolist()
            files_dn=np.loadtxt(os.path.join(ODIR,'SZAlist_dn.txt'),dtype='string',delimiter='\n').tolist()
    ...
    if len(Ifiles)>0:
        files_up=np.array(files_up)[Ifiles].tolist()
        files_dn=np.array(files_dn)[Ifiles].tolist()

    if filterBySza:
        np.savetxt(os.path.join(ODIR,'SZAlist_dn.txt'),files_dn,fmt='%s')
        np.savetxt(os.path.join(ODIR,'SZAlist_up.txt'),files_up,fmt='%s')
    ...
    if plotfitted:
        ...
        with tables.open_file(lp_path) as h5file:
            ...
            data_time = np.mean(h5file.get_node('/Time/UnixTime').read(),axis=1)
        fitted_plasma_frequency = np.sqrt(Ne*v_elemcharge**2/v_electronmass/v_epsilon0)/(2*np.pi*10.0**6)
        fitted_plasma_frequency_error = dNe*np.sqrt(v_elemcharge**2/v_electronmass/v_epsilon0)/(2*np.pi*10.0**6)/np.sqrt(Ne)/2.
        factor = 1
    ...
    while not done:
        ...
        for Ifile in range(len(tfiles_up)):
            ...
            if Ifile==0:
                if flipAuto:
                    flip=np.sign(output1['/Rx']['Frequency'][0,0]-output1['/Tx']['Frequency'][0,0])
                    print(flip)
                    if flip<0:
                        flip=1
                    else:
                ...
                try:
                    BeamcodeMap=np.array(output1['/Setup']['BeamcodeMap'])
                except:
                    az=np.mean(output1['/Antenna']['Azimuth'])
                    el=np.mean(output1['/Antenna']['Elevation'])
                    BeamcodeMap=np.array([[32768,az,el,0.0]])
        ...
        for jjj in range(Nbeams):
            ...
            if len(rrI)>=1:
                ...
                Ialt = np.where( (altSum[0] < alt_pl[:]) & (alt_pl[:] < altSum[-1]))
                ...
                tpl_spect_up=tpl_spect_up-np.repeat(tpl_spect_up[:,:,-1][:,:,np.newaxis],tpl_spect_up.shape[2],axis=2)
                '''
                tpl_spect_up_noise = np.repeat(np.mean(tpl_spect_up[:,:,-1+nalt:],axis=2)[:,:,np.newaxis],tpl_spect_up.shape[2],axis=2)*np.size(Ialt)
                #tpl_spect_up_noise = np.squeeze(np.sum(tpl_spect_up_noise[:,:,Ialt],axis=2))
                ...
                if dualpl:
                    ...
                    tpl_spect_dn=tpl_spect_dn-np.repeat(tpl_spect_dn[:,:,-1][:,:,np.newaxis],tpl_spect_dn.shape[2],axis=2)
                    '''
                    tpl_spect_dn_noise = np.repeat(np.mean(tpl_spect_dn[:,:,-1+nalt:],axis=2)[:,:,np.newaxis],tpl_spect_dn.shape[2],axis=2)*np.size(Ialt)
                    #tpl_spect_dn_noise = np.squeeze(np.sum(tpl_spect_dn_noise[:,:,Ialt],axis=3))
                ...
                if plotfitted:
                    ...
                    fp          = np.squeeze(fitted_plasma_frequency[:,jjj,alt_ind])
                ...
                while not_ok:
                    ...
                    if info=='':
                        ...
                        if (info=='u') or (info=='d'):
                            ...
                            pl_up = scipy.ndimage.filters.median_filter(pl_up, 7)
                            for i in range(3):
                                pl_up = scipy.ndimage.filters.gaussian_filter(
                                        pl_up,1, mode='nearest')
                            if dualpl:
                                pl_dn = scipy.ndimage.filters.median_filter(pl_dn, 7)
                                for i in range(3):
                                    pl_dn = scipy.ndimage.filters.gaussian_filter(
                                            pl_dn,1, mode='nearest')
                        elif (info=='flip'):
                            pl_up=np.fliplr(pl_up)
                            if dualpl:
                                pl_dn=np.fliplr(pl_dn)
                        elif (info[0:2]=='cf'):
                            factor=float(info[2:])
                        ...
                        if 1==1:
                            figg1.clf()
                            ax1 = figg1.add_subplot(topsubplot)
                            #pyplot.imshow(pl_up,vmin=blim[0],vmax=clim[0],
                            #       aspect='auto',origin='lower',
                            #       extent=[freq_up[0]/1e6,freq_up[-1]/1e6,0.0,
                            #               time_up[-1]-time_up[0]])
                            pm = ax1.pcolormesh(frup,tup,pl_up,vmin=blim[0],vmax=clim[0])
                            figg1.colorbar(pm)
